...
7. 协调模式与 `workflow-full`、`workflow-light` 正交，不能绕过流程与安全规则。
...
2. Leader 初始化或读取共享任务。
3. Leader 冻结正式契约并发布运行时快照。
4. Leader 根据依赖、路径所有权和并行规则派发子代理。
5. 子代理使用与手动角色相同的状态、blocker 和 handoff 协议。
6. Leader 汇合结果，核对实际改动和共享契约。
7. Leader 执行最小充分的整体验证并报告证据。
