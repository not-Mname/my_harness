...
## 何时使用（分级门禁，my_harness）

是否执行 TDD 由 Workflow 档位决定；档位不明确时先按 `using-superpowers` 读取 `~/.codex/rules/workflow-policy.md`。

- **轻量级：** 不强制 TDD。把唯一测试轮次用于信息量最高的验证，按顺序选择：现有端到端/集成测试 → 定向测试 → 构建/类型检查/lint；不得为扩大覆盖临时拼接多个独立测试目标。
- **中量级：** 高风险逻辑优先 TDD（红-绿-重构），其余按风险选择；测试轮次最多 3 轮。
- **重量级：** 完整执行本技能全部流程，轮次不额外限制，仍受平台与不可绕过底线约束。
...
**测试配额口径：** 测试轮次计入档位配额；一轮只能运行一个既有测试目标（框架内部多个用例允许），不得临时组合独立目标规避配额；修复后重新运行计为新轮次；lint、类型检查、构建和测试框架执行计入测试轮次，`git diff`、只读搜索和纯静态内容检查不计入。用户显式要求完整 TDD 时，放宽该循环必需的测试轮次并简短声明，不把整个任务提升为重量级。

**适用范围：** 下方铁律与红-绿-重构流程仅在按档位实际执行 TDD 的场景生效。
