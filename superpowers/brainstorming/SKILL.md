...
**分级门禁（my_harness）：** 流程深度由 Workflow 档位决定：
- 轻量级：压缩为必要澄清 + 简短方案确认，不写 spec、不生成计划文档；确认后直接实现。
- 中量级：合并需求澄清、方案比较和关键设计确认；多文件/跨模块或存在关键契约时写 spec，多步骤任务生成可压缩实施计划。
- 重量级：完整执行本技能全部步骤（含 spec 与用户书面审查）。

“未经用户确认设计不得进入实现”的门在所有档位均生效；轻量级/中量级不要求完整 spec。
</HARD-GATE>
...
3. **提出澄清问题** — 每次一个，了解目的/约束/成功标准（档位裁剪时可合并提问，遵守澄清配额）
4. **提出 2-3 种方案** — 附带权衡分析和你的推荐
5. **展示设计** — 按复杂度分节展示，每节展示后获得用户批准
6. **编写设计文档** — 保存到 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` 并 commit（轻量级跳过；中量级可选）
7. **规格自检** — 快速内联检查占位符、矛盾、模糊性、范围（详见下方）
8. **用户审查书面规格** — 在继续之前请用户审查规格文件（轻量级跳过；中量级可选）
9. **过渡到实现** — 调用 writing-plans 技能创建实现计划（轻/中量级确认后可直接实现，不强制 writing-plans）
...
**分级说明（my_harness）：** 本节（spec 文档、规格自检、用户审查关卡、writing-plans）仅重量级完整执行；中量级按需，轻量级跳过。
