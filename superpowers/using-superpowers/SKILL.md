...
技能调用由 Workflow 档位和档位裁剪表决定，一个技能是否适用于你的任务，由当前档位和裁剪表决定；档位未命中时不调用该技能。

如果你是作为子智能体被分派来执行特定任务的，忽略此技能。主智能体必须在派发前完成本入口的档位判定和流程路由。
</EXTREMELY-IMPORTANT>

## 规则
**技能调用：** 无论是什么档位在任务执行结束时都要给出若干下一个推荐的superpowers流程供用户选择，并默认使用你认为最适合你的技能。
