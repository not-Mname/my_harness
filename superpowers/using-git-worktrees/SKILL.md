...
**门禁（my_harness，默认禁止）：** `~/.codex/rules/git-development.md` 规定**未经允许不准创建 worktree、branch 或 tag**。使用本技能前必须通过 HITL Checkpoint（`[CP-n]`）逐项说明并获授权：worktree 创建、`.gitignore` 或项目设置修改及其提交、依赖安装与生命周期脚本、lockfile 或构建生成物变化、服务启动和基线测试。未获授权时**在当前目录原地工作**，不创建任何隔离区；平台已有原生隔离（如已处于功能分支）时不创建额外 worktree。
...
是否已获 HITL 授权创建隔离区（见上文门禁）？未获授权时原地工作并跳到步骤 2（项目设置）；已获授权时按步骤 1 继续。
