...
## Harness 门禁（my_harness）

派发前读取 `~/.codex/rules/workflow-policy.md` 与 `~/.codex/rules/multi-agent-assistance.md`，确认档位预算与协作规则。

- **档位配额**（单任务累计派发，含完成、失败、中断、返工）：轻量级可读写 1 个 / 只读 3 个；中量级可读写 3 个 / 只读 5 个；重量级不额外限制，仍受平台并发上限约束。
- **身份冻结**：派发时冻结只读或可读写身份；只读 Agent 不得升级为可读写。
- **子 Agent 不得继续派发**；不得自行集成、修改共享契约或扩大范围。
- **并行独立条件**（全部满足才并行）：目标和输出可单独验收；输入、依赖及 API/类型契约已确定；执行顺序不影响结果；写入范围不重叠；不共享可变状态、运行环境或互斥资源。
- **禁止并行**：后一任务依赖前一任务输出或多个现象同根因；修改同一文件、共享生成物或公共接口未定；Editor MCP 写入、场景或资源修改；数据迁移；Git 暂存、拉取、合并、提交等改变共享状态的操作；共用开发服务器、端口、数据库、缓存或构建目录；正在等待 HITL Checkpoint；派发成本高于直接处理。

与下方"何时使用"的独立性判断叠加：先满足本门禁，再按原流程执行。
