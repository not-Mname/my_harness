...
派发前读取 `~/.codex/rules/workflow-policy.md` 与 `~/.codex/rules/multi-agent-assistance.md`，确认档位预算。

- **档位配额**：轻量级一般不使用本技能（可读写 1 个配额）；中量级可读写 3 个 / 只读 5 个；重量级不额外限制。
- **串行派发**：每个实施 Agent 及其评审 Agent 串行执行，严禁同时派发多个实施 Agent；返工派发计入累计配额。
- **并行写入不得使用本技能**——确需并行时改用 `dispatching-parallel-agents`，且共享契约已冻结、写入路径互斥、运行环境隔离。
- **Git 约束**：实施 Agent 可按 `~/.codex/rules/git-development.md` 创建范围明确的普通本地提交，但不得 amend、改写历史或推送；不创建 worktree/branch/tag。
- **子 Agent 约束**：不扩大范围、不修改共享契约、不自行集成、不继续派发；越界需求只报告，由主 Agent 决定后续流程。
